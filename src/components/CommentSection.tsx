...
export function CommentSection({ contributionId }: { contributionId: string }) {
  ...
  const { items, page, total, totalPages, stale, loading, loadFailed, error, reload, refresh, goToPage } = usePagedList<CommentItem>({
  ...
  return (
    <section className={styles.section} aria-labelledby="comment-section-title">
      ...
      {/* 不要再套一层包裹 div：Pagination 会把自己的父元素当作「被分页的区域」
          滚动定位，多一层 wrapper 就会把评论列表顶出视口（组件自身在 total=0 时返回 null）。 */}
      <Pagination
        page={page}
