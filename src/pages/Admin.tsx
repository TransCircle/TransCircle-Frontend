...
export const Admin = () => {
  ...
  /**
   * 取审核历史的指定页。需 contribution:audit:read（api.md §6.7）：reviewer 无该
   * 权限时不发起必 403 的无谓请求，历史区保持为空。
   * 沿用 detailSeq 作竞态守卫：换详情或返回列表都会让在途响应作废，不会把 A 的
   * 历史盖到 B 上。
   */
  const loadReviewEvents = async (contributionId: string, targetPage: number) => {
    ...
    if (seq !== detailSeq.current) return // 过期响应，丢弃（loading 交给新请求收尾）
    if (result.ok) {
  ...
  if (!selected) {
    ...
    // 总数来自服务端 pagination.total，不再是「当前这一页的条数」
