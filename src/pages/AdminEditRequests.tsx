...
export const AdminEditRequests = () => {
  ...
  const { items, page, total, totalPages, stale, staleResults, loading, error, setError: setListError, reload, refresh, goToPage } = usePagedList<EditRequestItem>({
    fetchPage: async (targetPage) => {
      const params = new URLSearchParams({ limit: String(PAGE_SIZE), page: String(targetPage), status: statusFilter })
